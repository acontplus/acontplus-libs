...
(function () {
  DateRangePicker = function (element, options, cb) {
    ...
    if (this.applyButtonClasses.length) jq.addClass(applyBtnEl, this.applyButtonClasses);
    if (this.cancelButtonClasses.length) jq.addClass(cancelBtnEl, this.cancelButtonClasses);
  ...
  DateRangePicker.prototype = {
    ...
    updateCalendars: function () {
      ...
      if (this.timePicker) {
        ...
        if (this.endDate) {
          ...
          if (isNaN(minute)) {
            minute = parseInt(
              jq.findLast(this.container.querySelector('.left .minuteselect')).value,
              10,
            );
          }
          ...
          if (isNaN(minute)) {
            minute = parseInt(
              jq.findLast(this.container.querySelector('.right .minuteselect')).value,
              10,
            );
          }
          second = this.timePickerSeconds
      ...
      for (let i = 0; i < rangesLiElList.length; ++i) rangesLiElList[i].classList.remove('active');
      ...
      if (side == 'left') {
        ...
        if (timeSelector.innerHTML != '') {
          selected.hour(
            !isNaN(selected.hour())
              ? selected.hour()
              : jq.findSelectedOption(timeSelector.querySelector('.hourselect')).value,
          );
          selected.minute(
            !isNaN(selected.minute())
              ? selected.minute()
              : jq.findSelectedOption(timeSelector.querySelector('.minuteselect')).value,
          );
          selected.second(
            !isNaN(selected.second())
              ? selected.second()
              : jq.findSelectedOption(timeSelector.querySelector('.secondselect')).value,
          );
          if (!this.timePicker24Hour) {
    ...
    outsideClick: function (e) {
      ...
      this.hide();
      this.element.dispatchEvent(
        new CustomEvent('outsideClick.daterangepicker', { bubbles: true, detail: this }),
      );
    },
    ...
    showCalendars: function () {
      ...
      this.move();
      this.element.dispatchEvent(
        new CustomEvent('showCalendar.daterangepicker', { bubbles: true, detail: this }),
      );
    },
    ...
    hideCalendars: function () {
      ...
      this.element.dispatchEvent(
        new CustomEvent('hideCalendar.daterangepicker', { bubbles: true, detail: this }),
      );
    },
    ...
    clickDate: function (e) {
      ...
      if (this.endDate || date.isBefore(this.startDate, 'day')) {
        ...
        if (this.timePicker) {
          ...
          if (isNaN(minute)) {
            minute = parseInt(
              this.container.querySelector('.left .minuteselect option:last').value,
              10,
            );
          }
        ...
        if (this.timePicker) {
          ...
          if (isNaN(minute)) {
            minute = parseInt(
              this.container.querySelector('.right .minuteselect option:last').value,
              10,
            );
          }
          var second = this.timePickerSeconds
    ...
    calculateChosenLabel: function () {
      ...
      for (let i = 0; i < rangesKey.length; ++i) {
        ...
        if (this.timePicker) {
          ...
          if (
            this.startDate.format('YYYY-MM-DD') == range[0].format('YYYY-MM-DD') &&
    ...
    clickApply: function (e) {
      ...
      e.target.dispatchEvent(
        new CustomEvent('apply.daterangepicker', { bubbles: true, detail: this }),
      );
    },
    ...
    clickCancel: function (e) {
      ...
      this.hide();
      e.target.dispatchEvent(
        new CustomEvent('cancel.daterangepicker', { bubbles: true, detail: this }),
      );
    },
  ...
  var jq = {
    ...
    offSub: function (el, event, listener) {
      if (typeof el.length === 'number')
        for (let i = 0; i < el.length; ++i) el[i].removeEventListener(event, listener);
    ...
    onSub: function (el, event, listener) {
      if (typeof el.length === 'number')
        for (let i = 0; i < el.length; ++i) el[i].addEventListener(event, listener);

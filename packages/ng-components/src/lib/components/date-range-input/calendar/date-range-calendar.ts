/**
 * AcpDateRangeCalendar Component
 *
 * Custom calendar component that manages two side-by-side month views
 * with support for:
...
import { ACP_ACTIVE_DATE_DEBOUNCE } from '../constant/date-filter-const';

@Component({
  selector: 'acp-date-range-calendar',
  templateUrl: './date-range-calendar.html',
  styleUrls: ['./date-range-calendar.scss'],
  imports: [MatCalendar],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class AcpDateRangeCalendar implements AfterViewInit {
  firstViewStartDate = signal(new Date());
  secondViewStartDate = signal(getDateOfNextMonth(this.firstViewStartDate()));
  secondViewMinDate = signal(getFirstDateOfNextMonth(this.firstViewStartDate()));
  ...
  private isAllowHoverEvent = false;
  ...
  set selectedDates(selectedDates: DateRange<Date> | null) {
    ...
    const computedEndDate =
      startDate.getMonth() === endDate.getMonth() ? getDateOfNextMonth(endDate) : endDate;
   ...
   */
  private onFirstViewActiveDateChange(activeDate: AcpActiveDate): void {
    ...
    setTimeout(handler, ACP_ACTIVE_DATE_DEBOUNCE);
  }
   ...
   * @param activeDate - Object containing `previous` and `current` date values.
   */
  private onSecondViewActiveDateChange(_activeDate: AcpActiveDate): void {
    this.attachHoverEvent('secondCalendarView');
  }
   ...
   */
  private handleFirstViewNextEvent(currDate: Date, _force?: boolean): void {
   ...
   * @param activeDate - Object containing `previous` and `current` date values.
   */
  private handleFirstViewPrevEvent(activeDate: AcpActiveDate): void {
    if (this.firstCalendarView.currentView.toLocaleLowerCase() !== 'month') {
      return;
    }
   ...
   */
  private isPrevious(activeDate: AcpActiveDate): boolean {
  ...
  private attachHoverEvent(viewId: string) {
    ...
    setTimeout(() => this.addHoverEvents(nodes), ACP_ACTIVE_DATE_DEBOUNCE);
  }
   ...
   */
  private removeDefaultFocus(classRef: AcpDateRangeCalendar): void {
    setTimeout(() => {
      const btn: HTMLButtonElement[] = classRef.el.nativeElement.querySelectorAll(

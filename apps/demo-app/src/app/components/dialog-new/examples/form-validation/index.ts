...
const appHtml = `<h2>Form validation</h2>

<p>
  A dialog with a reactive form. The declarative Save action is disabled while the form is invalid.
</p>

<section class="actions">
  <button matButton="elevated" color="primary" (click)="openForm()">Open form dialog</button>
</section>
...
export class App {
  ...
  openForm(): void {
    const ref = this.dialogs.open<UserFormDialog, { user: UserFormData }, UserFormData>(
      UserFormDialog,
      {
        title: 'User form',
        size: 'md',
  ...
  template: \`
    <acp-dialog>
      ...
      <acp-dialog-actions align="end">
        ...
        <acp-button text="Save" color="success" [disabled]="form.invalid" (clicked)="save()" />
      </acp-dialog-actions>
    </acp-dialog>
  \`,
  changeDetection: ChangeDetectionStrategy.Eager,
...
const dialogNewFormValidationExampleConfig = {
  title: 'Form validation',
  description:
    'A reactive form dialog where the declarative Save action is disabled while the form is invalid.',
  component: App,
  files: [
    {
      file: 'app.html',
...
export { dialogNewFormValidationExampleConfig };

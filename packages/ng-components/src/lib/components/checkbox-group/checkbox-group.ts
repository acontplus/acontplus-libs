...
import { AcpCheckboxGroupOption } from './interfaces';
import { AcpToObservablePipe } from './../../../core/pipes';

export class AcpCheckboxBase {
  constructor(
    public label?: any,
    public value?: any,
  ) {}
...
@Component({
  selector: 'acp-checkbox-group',
  exportAs: 'acpCheckboxGroup',
  host: {
    class: 'acp-checkbox-group',
  },
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => AcpCheckboxGroup),
      multi: true,
    },
  ],
  imports: [FormsModule, MatCheckbox, AcpToObservablePipe, AsyncPipe],
})
export class AcpCheckboxGroup implements AfterViewInit, OnDestroy, ControlValueAccessor {
  ...
  set items(value: any[]) {
    ...
    this._items = value.map(option => {
      return option instanceof Object ? { ...option } : new AcpCheckboxBase(option, option);
    });
  ...
  selectionChange = output<{ model: AcpCheckboxGroupOption[]; index: number }>();

  selectAll = false;
  selectAllIndeterminate = false;

  selectedItems: AcpCheckboxGroupOption[] = [];

  _onChange: (value: AcpCheckboxGroupOption[]) => void = () => null;
   ...
   */
  private _selectValue(value: AcpCheckboxGroupOption) {
    const correspondingOption = (this.items as AcpCheckboxGroupOption[]).find(option => {
      try {
   ...
   */
  registerOnChange(fn: (value: AcpCheckboxGroupOption[]) => Record<string, unknown>): void {
    this._onChange = fn;
  }
  ...
  private _checkMasterCheckboxState() {
    if (
      (this.items as AcpCheckboxGroupOption[])
  ...
  private _getSelectedItems(index: number) {
    this.selectedItems = (this.items as AcpCheckboxGroupOption[]).filter(option => option.checked);

    if (this._compareWith) {
      this.selectedItems = (this._originalItems as AcpCheckboxGroupOption[]).filter(option =>
    ...
    this.selectionChange.emit({ model: this.selectedItems, index });
  }

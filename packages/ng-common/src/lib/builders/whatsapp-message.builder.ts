...
export class WhatsAppMessageBuilder {
  ...
  private buildMainMessage(data: DocumentDeliveryData): string {
  ...
  private buildPromoMessage(establishmentName: string): string {
    return (
  ...
  private getDocumentTypeDisplay(tipo: string): string {
